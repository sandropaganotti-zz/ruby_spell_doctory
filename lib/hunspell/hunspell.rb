module Hunspell
  class Hunspell
    ...
    def respawn_handler(dictionary)
      ...
      @hunspell = Wrapper.create(@dicts[dictionary]['aff'],@dicts[dictionary]['dic'])
    end
    ...
    def suggest(word)
      ...
      if (suggestion_pointer.null?) then [] 
      else suggestion_pointer.get_array_of_string(0,suggestions_number).compact 
      end
    end
    ...
    def bootstrap(config_obj)
      raise BootstrapError.new("No configuration info supplied to Hunspell 
            constructor and no default configuration file available") if config_obj.nil? 
      return config_obj['dictionaries'].merge({
          'default' => config_obj['dictionaries'][config_obj['dictionaries']['default']]
      })
    end
